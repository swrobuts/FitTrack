/**
 * FitTrack-Lab · Laufzeit der Lernumgebung
 *
 * Zustaendig fuer:
 ...
 *   - eine echte SQLite-Datenbank im Browser (sql.js, WebAssembly) mit dem Schema der App
...
const SPRACHSCHLUESSEL = 'fittrack:sprache'
const OSSCHLUESSEL = 'fittrack:os'
...
function setzeSprache (lang) {
  ...
  document.dispatchEvent(new CustomEvent('fittrack:sprache', { detail: { lang } }))
}
...
function setzeOs (os) {
  ...
  document.dispatchEvent(new CustomEvent('fittrack:os', { detail: { os } }))
}
...
const T = {
  ...
  fehlerSql:    { de: 'SQLite meldet einen Fehler', en: 'SQLite reports an error' },
  dbLaden:      { de: 'Die Datenbank wird gestartet …', en: 'Starting the database …' },
  dbBereit:     { de: 'SQLite läuft im Browser, Schema und Daten wie in der App', en: 'SQLite is running in your browser with the app schema and data' },
  ...
  typ: {
    ...
    sql:        { de: 'SQL auf FitTrack', en: 'SQL on FitTrack' }
  }
}
...
const TERMINAL_HINWEISE = {
  ...
  code:           { de: '"code ." öffnet den aktuellen Ordner in Visual Studio Code. In WebStorm: File → Open und den Ordner wählen.', en: '"code ." opens the current folder in Visual Studio Code – the dot is the directory, not punctuation.' },
  ...
  abbildUnbekannt:{ de: 'Diese Nachbildung kennt nur wenige Abbilder: hello-world, python, nginx, postgres, ubuntu und das selbst gebaute fittrack (docker build -t fittrack . im Ordner FitTrack).', en: 'This model knows only a few images: hello-world, postgres, nginx, python, adminer, ubuntu, n8nio/n8n.' },
...
const LABS = [
  ...
  { id: 'lab-01', nr: '01', datei: 'lab-01-health.html', uebungen: 3,
    titel: { de: 'US-3: Der Health-Endpunkt', en: 'US-3: the health endpoint' }, voraussetzung: { de: 'Lab 00', en: 'Lab 00' },
    zeit: { de: '30 Minuten', en: '30 minutes' },
    ziel: { de: 'Die Schleife Test rot, Prompt, Prüfen, Code, Test grün, Commit einmal vollständig durchlaufen.', en: 'Run the loop test red, prompt, review, code, test green, commit once in full.' } },
  { id: 'lab-02', nr: '02', datei: 'lab-02-daten.html', uebungen: 5,
    titel: { de: 'US-1: Datenhaltung und Workouts-API', en: 'US-1: data layer and workouts API' }, voraussetzung: { de: 'Lab 01', en: 'Lab 01' },
    zeit: { de: '60 Minuten', en: '60 minutes' },
    ziel: { de: 'Das Datenmodell lesen, die Sicht abfragen und die Datenzugriffsschicht von der API trennen.', en: 'Read the data model, query the view and separate the data layer from the API.' } },
  { id: 'lab-03', nr: '03', datei: 'lab-03-kennzahlen.html', uebungen: 4,
    titel: { de: 'US-2: Kennzahlen und Erwartungswerte', en: 'US-2: metrics and expected values' }, voraussetzung: { de: 'Lab 02', en: 'Lab 02' },
    zeit: { de: '45 Minuten', en: '45 minutes' },
    ziel: { de: 'Erwartungswerte selbst rechnen, Randfälle testen und Rechenlogik ohne Framework kapseln.', en: 'Compute expected values yourself, test edge cases and isolate calculation logic.' } },
  { id: 'lab-04', nr: '04', datei: 'lab-04-frontend.html', uebungen: 4,
    titel: { de: 'US-4 bis US-6: Das Dashboard', en: 'US-4 to US-6: the dashboard' }, voraussetzung: { de: 'Lab 03', en: 'Lab 03' },
    zeit: { de: '75 Minuten', en: '75 minutes' },
    ziel: { de: 'Ein mobile-first Frontend bauen, das jede Zahl aus der API holt und nichts hartkodiert.', en: 'Build a mobile-first frontend that fetches every number from the API and hardcodes nothing.' } },
  { id: 'lab-05', nr: '05', datei: 'lab-05-tests.html', uebungen: 4,
    titel: { de: 'Testkonzept und Abnahme', en: 'Test concept and acceptance' }, voraussetzung: { de: 'Lab 03', en: 'Lab 03' },
    zeit: { de: '45 Minuten', en: '45 minutes' },
    ziel: { de: 'Testpyramide, Definition of Done und Abnahmekriterien auf den eigenen Code anwenden.', en: 'Apply the test pyramid, definition of done and acceptance criteria to your own code.' } },
  { id: 'lab-06', nr: '06', datei: 'lab-06-docker.html', uebungen: 4,
    titel: { de: 'Docker: Image und Container', en: 'Docker: image and container' }, voraussetzung: { de: 'Lab 04', en: 'Lab 04' },
    zeit: { de: '45 Minuten', en: '45 minutes' },
    ziel: { de: 'Die App als Image bauen, als Container starten und vom Smartphone aus prüfen.', en: 'Build the app as an image, run it as a container and check it from a phone.' } },
  { id: 'lab-07', nr: '07', datei: 'lab-07-deploy.html', uebungen: 3,
    titel: { de: 'Render und GitHub Actions', en: 'Render and GitHub Actions' }, voraussetzung: { de: 'Lab 06', en: 'Lab 06' },
    zeit: { de: '30 Minuten', en: '30 minutes' },
    ziel: { de: 'Aus einem Push eine öffentliche Adresse machen und verstehen, was dabei wo läuft.', en: 'Turn a push into a public address and understand what runs where.' } },
  { id: 'lab-08', nr: '08', datei: 'lab-08-ki-workflow.html', uebungen: 4,
    titel: { de: 'Der KI-Workflow', en: 'The AI workflow' }, voraussetzung: { de: 'Lab 01', en: 'Lab 01' },
    zeit: { de: '45 Minuten', en: '45 minutes' },
    ziel: { de: 'Prompts mit Kontext und Randbedingungen schreiben und generierten Code wie einen fremden Pull Request prüfen.', en: 'Write prompts with context and constraints and review generated code like a stranger\'s pull request.' } },
...
function zwei (knoten, wert, eigenschaft = 'textContent') {
  ...
  setze()
  document.addEventListener('fittrack:sprache', setze)
  return knoten
}
...
function baueBefehl (ziel, def) {
  ...
  zeichne()
  document.addEventListener('fittrack:os', zeichne)
  document.addEventListener('fittrack:sprache', zeichne)
...
function baueTerminal (ziel, opt = {}) {
  ...
  if (!fest) {
    document.addEventListener('fittrack:os', () => {
      welt.os = aktuellesOs()
      zeichneKopf()
  ...
  document.addEventListener('fittrack:sprache', zeichneAuftrag)
...
/**
 * SQLite im Browser über sql.js (WebAssembly). Schema und Daten sind dieselben
 * wie in der App: data/schema.sql und data/fittrack.sql (Export der
 * Datenbank). Abfragen laufen vollständig im Browserfenster.
 */
let dbVersprechen = null
let sqlModul = null

async function ladeSqlJs () {
  if (sqlModul) return sqlModul
  const basis = 'https://cdn.jsdelivr.net/npm/sql.js@1.13.0/dist/'
  await new Promise((erfuellt, abgelehnt) => {
    if (window.initSqlJs) return erfuellt()
    const sc = document.createElement('script')
    sc.src = basis + 'sql-wasm.js'
    sc.onload = erfuellt
    sc.onerror = () => abgelehnt(new Error('sql.js konnte nicht geladen werden'))
    document.head.append(sc)
  })
  sqlModul = await window.initSqlJs({ locateFile: (f) => basis + f })
  return sqlModul
}
...
async function holeSaat () {
  if (saatText == null) {
    const teile = []
    for (const datei of ['data/schema.sql', 'data/fittrack.sql']) {
      const r = await fetch(url(datei))
      if (!r.ok) throw new Error(datei + ': ' + r.status)
      teile.push(await r.text())
    }
    saatText = teile.join('\n')
  }
  return saatText
...
function alsErgebnis (teile) {
  const letztes = [...teile].reverse().find(t => t.columns && t.columns.length) || teile[teile.length - 1]
  if (!letztes) return { fields: [], rows: [] }
  return { fields: letztes.columns.map(name => ({ name })), rows: letztes.values }
}
...
function baueDbBand (ziel) {
  ...
  const setzen = async () => {
    ...
    try {
      ...
      btn.disabled = false
      document.dispatchEvent(new CustomEvent('fittrack:datenbank'))
    } catch (e) {
...
function baueBox (uebung, ctx) {
  ...
  zeichneAufgabe()
  document.addEventListener('fittrack:sprache', zeichneAufgabe)
  ...
  if (uebung.typ === 'sql') {
    ...
    if (uebung.hinweis) {
      ...
      zeichne(); document.addEventListener('fittrack:sprache', zeichne)
      d.append(inh)
      koerper.append(d)
    }
    if (uebung.loesung) {
    ...
    const fuehre = async (db, sql) => alsErgebnis(db.exec(sql))
...
function initTitel () {
  const en = document.querySelector('meta[name="fittrack:titel-en"]')?.content
...
function karteFortschritt (nurLab = null) {
  ...
  zeichne()
  document.addEventListener('fittrack:fortschritt', zeichne)
  document.addEventListener('fittrack:sprache', zeichne)
}
...
async function starteLab (labId) {
  ...
  for (const halter of document.querySelectorAll('[data-sql-konsole]')) {
    const box = baueBox({
      ...
      aufgabe: {
        de: '<p>Schreiben Sie eine beliebige Abfrage gegen die FitTrack-Datenbank (Tabellen sportart und workout, Sicht v_workout). Nichts hier wirkt über diesen Browser hinaus.</p>',
        en: '<p>Write any query against the sample database. Nothing here has any effect beyond this browser.</p>'
      },
      start: halter.dataset.start || 'SELECT * FROM v_workout ORDER BY datum DESC LIMIT 10;'
...
/**
 * Kopierknopf an Prompt- und Codeblöcken: jedes <pre data-kopieren> bekommt
 * eine Leiste mit Beschriftung (data-kopieren="Prompt") und Knopf.
 */
function initKopierbloecke () {
  for (const pre of document.querySelectorAll('pre[data-kopieren]')) {
    const huelle = el('div', 'kopierblock')
    pre.parentNode.insertBefore(huelle, pre)
    const leiste = el('div', 'kopierblock-leiste')
    leiste.append(el('span', 'kopierblock-titel', pre.dataset.kopieren || 'Code'))
    const echo = el('span', 'hinweis-klein')
    const btn = zwei(el('button', 'btn-mini'), T.kopieren)
    btn.type = 'button'
    btn.addEventListener('click', () => kopiere(pre.textContent, echo))
    leiste.append(el('span', 'spacer'), echo, btn)
    huelle.append(leiste, pre)
  }
}
