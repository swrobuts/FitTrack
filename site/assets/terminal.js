...
export const HEIMAT = () => ordner({
  Dokumente: ordner({
    'notizen.txt': datei('Termine FitTrack-Projekt\nSprint 1: API\nSprint 2: Frontend\nSprint 3: Docker und Abnahme\n')
  }),
  FitTrack: ordner({
    'README.md': datei('# FitTrack\nMobile-first Fitness-Dashboard, Lehrprojekt Wirtschaftsinformatik.\n'),
    'requirements.txt': datei('fastapi>=0.115\nuvicorn[standard]>=0.30\npytest>=8.0\nhttpx2>=2.0\n'),
    'Dockerfile': datei('FROM python:3.12-slim\nWORKDIR /app\nCOPY requirements.txt .\nRUN pip install --no-cache-dir -r requirements.txt\nCOPY backend/ ./backend/\nCOPY frontend/ ./frontend/\nEXPOSE 8000\nCMD ["sh", "-c", "uvicorn backend.app.main:app --host 0.0.0.0 --port ${PORT:-8000}"]\n'),
    'docker-compose.yml': datei('services:\n  fittrack:\n    build: .\n    image: fittrack\n    ports:\n      - "${FITTRACK_PORT:-8000}:8000"\n'),
    'render.yaml': datei('services:\n  - type: web\n    name: fittrack\n    runtime: docker\n    plan: free\n    healthCheckPath: /health\n'),
    backend: ordner({
      app: ordner({
        'main.py': datei('from fastapi import FastAPI\napp = FastAPI(title="FitTrack")\n'),
        'daten.py': datei('import sqlite3\n'),
        'statistik.py': datei(''),
        'models.py': datei(''),
        data: ordner({ 'fittrack.db': datei(''), 'schema.sql': datei('CREATE TABLE sportart (...);\nCREATE TABLE workout (...);\nCREATE VIEW v_workout AS ...;\n') })
      }),
      tests: ordner({ 'conftest.py': datei(''), 'test_api.py': datei('def test_health(): ...\n'), fixtures: ordner({ 'workouts_klein.sql': datei('') }) })
    }),
    frontend: ordner({ 'index.html': datei('<!doctype html>\n'), 'style.css': datei(''), 'app.js': datei('') })
  })
})
...
function docker (w, marken) {
  ...
  if (unter === 'build') {
    ...
    return ok(['[+] Building 14.8s (11/11) FINISHED',
      ...
      ' => [4/6] RUN pip install --no-cache-dir -r requirements.txt 9.6s',
      ' => [5/6] COPY backend/ ./backend/                          0.1s',
      ' => [6/6] COPY frontend/ ./frontend/                        0.0s',
  ...
  if (unter === 'compose') {
    ...
    const inhalt = (k.kinder['compose.yaml'] || k.kinder['docker-compose.yml']).inhalt || ''
    if (arg[0] === 'up' && inhalt.includes('fittrack')) {
      // FitTrack-Compose: ein Dienst, gebaut aus dem Dockerfile im Ordner
